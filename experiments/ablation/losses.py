...
from experiments.ablation.config import AblationConfig
...
class AblationHybridLoss(nn.Module):
    ...
    def forward(
        ...
        if self.config.use_consistency_loss:
            consistency_loss = self._compute_consistency_loss(
                pred_ratio, edge_index, edge_attr
            )
    ...
    def compute_density_loss(
        ...
        graph_density_target = torch.matmul(
            condition, self.target_densities.to(condition.device)
        )
