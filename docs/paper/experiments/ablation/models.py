...
from torch_geometric.nn import BatchNorm, GATv2Conv, GCNConv, GINConv, SAGEConv

from docs.paper.experiments.ablation.config import AblationConfig, ConditioningMethod, GNNBackbone
...
class AblationShearWallGNN(nn.Module):
    ...
    def __init__(
        self, config: AblationConfig, node_in_dim: int = 25, edge_in_dim: int = 8, cond_dim: int = 3
    ):
        super().__init__()
        ...
        if self.conditioning_type == ConditioningMethod.FILM:
            self.film_layers = nn.ModuleList(
                [FiLMLayer(self.hidden_dim, self.cond_emb_dim) for _ in range(self.num_layers)]
            )
...
def create_model_from_config(
    config: AblationConfig, node_in_dim: int = 25, edge_in_dim: int = 8
) -> nn.Module:
    """
    根据消融配置创建模型
