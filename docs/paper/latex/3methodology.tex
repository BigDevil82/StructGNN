...
\caption{The overall framework of the proposed room-level conditional shear wall layout generation method.}
\label{fig:framework}
\end{figure*}
...
\textcolor{red}{$\mathbf{M} \in \{0,1\}^{N \times 16}$} & \textcolor{red}{Graph-level buildable mask formed by stacking the per-room masks} \\
...
\textbf{Step 3: Adjacency Edge Construction.} \textcolor{red}{For each pair of rooms, the shared physical boundary length $l_{ij}$ is computed. An adjacency edge $e_{ij}$ is added to $\mathcal{E}$ only if $l_{ij}$ exceeds a physical threshold $\tau_{adj}$, which is set to 400~mm in this study. This value is not prescribed by a design code; it is an empirical threshold selected from the geometric scale of the floor plans and shear wall layouts in the dataset. Shared contacts shorter than approximately 400~mm are typically minor corner contacts or drafting artifacts and are unlikely to provide a meaningful boundary for wall placement, so they are excluded from graph connectivity.}
...
\item \textcolor{red}{Graph-level buildable mask $\mathbf{M} \in \{0,1\}^{N \times 16}$, formed by stacking the per-room masks $\mathbf{m}_i$}
\end{enumerate}
