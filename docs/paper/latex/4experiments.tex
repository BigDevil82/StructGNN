...
\midrule
Component-level GNN & 244.80 $\pm$ 74.68 & 444.34 $\pm$ 133.08 & 0.06 s & 86 s  & 581 MB \\
Room-level GNN & 26.46 $\pm$ 8.69 & 71.61 $\pm$ 29.58 & 0.12 s & 103 s & 981 MB \\
Reduction & 89.2\% & 83.9\% & -100.0\% & -19.8\% & -68.8\% \\
\bottomrule
\end{tabular*}
\par\smallskip
{\small\noindent\parbox{\textwidth}{\raggedright Note: Node and edge statistics are computed on 112 training floor plans. Negative reduction values indicate that the room-level implementation requires more time or memory than the component-level baseline for the measured item.}}
\endgroup
\end{table*}

\textcolor{red}{Table~\ref{tab:efficiency} provides a quantitative comparison of graph complexity and computational efficiency. The room-level representation reduces the average number of nodes from 244.80 to 26.46 and the average number of edges from 444.34 to 71.61, corresponding to reductions of 89.2\% and 83.9\%, respectively. This directly supports the claim that representing rooms rather than low-level wall components produces a more compact graph. However, the current implementation does not yet translate this compactness into lower measured runtime or memory usage: inference time, training time per epoch, and GPU memory are higher than those of the component-level baseline. This suggests that the computational benefit of the room-level abstraction depends not only on graph size but also on implementation details, model width, and batching efficiency.}
...
\midrule
Equal weights $(1/3,1/3,1/3)$ & 0.701 & 0.700 & 0.692 & 0.651 \\
Density-oriented $(0.5,0.25,0.25)$ & 0.713 & 0.713 & 0.702 & 0.640 \\
Geometry-oriented $(0.25,0.5,0.25)$ & 0.661 & 0.652 & 0.650 & 0.624 \\
Uniformity-oriented $(0.25,0.25,0.5)$ & 0.729 & 0.735 & 0.725 & 0.690 \\
\bottomrule
\end{tabular*}
\endgroup
\end{table*}

\textcolor{red}{A CGS weight-sensitivity analysis is reported in Table~\ref{tab:cgs_sensitivity}. Under equal weights and geometry-oriented weights, the full model obtains the highest CGS among the selected variants; under density-oriented weights, it remains nearly tied with the highest selected variant. When uniformity is assigned the largest weight, the early-concatenation variant obtains a slightly higher CGS because it produces smoother but less geometrically accurate layouts. This result indicates that CGS rankings can be affected when the uniformity term is over-emphasized, but the full model remains the most balanced setting across condition compliance, matched-condition geometry, and spatial regularity. In particular, the full model consistently outperforms the variants without dual-stream training and without density loss under all tested weight settings.}
...
For both cases, room-based predictions are first converted into vector structural members and then imported into structural analysis software to build finite-element (FE) models. Response-spectrum analysis is conducted to obtain inter-story drift ratios and displacement responses, consistent with standard seismic evaluation practice for building structures \cite{GB500112010Code2016,qianDesignTallBuilding2018}. We compare two engineering indicators: the story-wise drift ratio distribution and the maximum vertical displacement of each floor.
