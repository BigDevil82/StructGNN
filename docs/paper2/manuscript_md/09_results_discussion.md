...
图 4.1 总结了两个全局代理模型在测试集上的表现。对于可行性代理模型，图 4.1(a) 给出了整体分类性能。LayoutParamGNN 在测试集上取得 ROC-AUC 为 0.9693、PR-AUC 为 0.8593，说明模型能够较好地区分可行与不可行候选方案。在面向筛选的阈值设置下，模型能够提前拒绝约 47.36% 的候选样本，同时保持 0.9998 的 feasible recall，仅误拒 3 个真实可行样本。这一结果表明，全局可行性代理模型已经具备作为保守预筛选器的潜力。

然而，图 4.1(b) 显示该模型在布局层面仍存在明显概率偏差。每个点对应一个测试布局，横轴为该布局中真实可行样本比例，纵轴为模型预测的平均可行性概率。理想情况下，各布局应接近对角线；实际结果中，部分布局的预测概率明显高于真实可行率。例如若干 Group8 高需求布局具有较低真实可行率，但模型给出的平均可行概率偏高。这说明全局分类模型虽然能够在样本层面进行排序和筛选，但其概率输出在不同布局之间并不完全校准。若直接将该概率作为可靠的结构可行性判断，可能导致过度乐观的筛选决策。

钢筋用量代理模型的整体回归结果见图 4.1(c)。与仅使用设计参数的 MLP 相比，加入全局布局统计特征显著降低误差，而 room-graph LayoutParamGNN 进一步取得最佳性能。三类模型的测试 MAE 分别为 32.3 t、13.8 t 和 11.5 t，对应 $R^2$ 分别为 0.351、0.891 和 0.924。该结果说明钢筋用量不仅由截面和设计条件决定，还强烈依赖布局几何和拓扑信息；相比人工统计特征，room graph 表征能够提供额外的布局信息。

钢筋模型还表现出较强的排序能力。LayoutParamGNN 的 pairwise ranking accuracy 为 0.9787，Spearman 相关系数为 0.9779，top-10% recall 为 0.9483。这与本文的优化应用目标相吻合，因为钢筋代理模型主要用于候选方案排序，而不是直接给出最终材料用量。尽管如此，图 4.1(d) 显示其误差并非均匀分布。在最高 20% 钢筋用量区间，MAE 增加到约 20.7 t，bias 达到 -17.4 t，表明模型对高钢筋需求样本存在系统性低估。该偏差会影响候选造价排序，尤其是在复杂布局或高需求设计条件下更为明显。
...
这些结果验证了本文在线局部校准的基本假设：全局代理模型在跨布局预测中不可避免存在偏差，但优化过程中自然产生的少量 FEA 样本可以用于快速修正当前布局上的概率和残差误差。经过校准后，代理模型更适合用于筛选和排序，但最终结构评价仍需由真实 FEA 完成。
...
综合来看，漏斗图说明了 FEA 调用减少的来源，优化轨迹图揭示了可行性筛选与造价预排序的互补作用，布局级热力图则表明优化成功率受到布局难度和搜索空间限制的影响。这些结果支持本文的核心观点：校准代理模型不应被视为 FEA 的替代品，而应作为昂贵结构分析预算的分配工具。最终设计仍需通过物理分析和规范校核确认，但代理模型能够显著减少优化过程中不必要的真实分析调用。
