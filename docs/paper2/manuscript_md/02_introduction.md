...
因此，本文关注的问题不是训练一个能够完全取代有限元分析的预测模型，而是研究如何将存在误差的代理模型转化为有限元预算分配工具。代理模型如果能够识别明显不可行的候选方案，或能够对剩余候选方案的相对优劣给出足够有效的排序，就可以帮助优化算法减少无效有限元分析，将真实分析优先分配给更有希望的设计区域。与直接使用代理模型给出最终设计结果相比，这种思路保留了最终方案的真实有限元验证和规范校核，因此更符合工程设计对可靠性的要求。
