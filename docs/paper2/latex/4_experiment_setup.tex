...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{model_cases.pdf}
...
Two global surrogate models are trained, i.e. the feasibility surrogate $\Phi_f$ and the reinforcement usage surrogate $\Phi_s$. They use the same candidate representation and model architecture, but are trained separately with independent parameters for different prediction tasks. The feasibility surrogate outputs the probability that a candidate can pass the comprehensive structural checks, while the reinforcement surrogate predicts the reinforcement quantity before true evaluation.

To investigate how different levels of layout information affect surrogate performance, three models with progressively enriched candidate representations are compared. Param-MLP uses only section, material, and design-condition parameters and therefore serves as a baseline without explicit layout information. Stat-MLP augments these inputs with fixed-dimensional global layout statistics, including the plan aspect ratio, shear wall ratio, total wall length, wall centroid, and other geometric summaries. The proposed FuseGNN model further incorporates the room-level graph, which jointly fuses room-level topology, global layout descriptors, and candidate design parameters. This enables the surrogate to capture both the global geometric context and local spatial relationships that influence structural feasibility and reinforcement demand.

The FuseGNN adopts a three-layer GraphSAGE encoder for room-level graph feature extraction\cite{Hamilton2017GraphSAGE}. In each surrogate model, the graph-level representation is fused with global layout statistics and design-parameter embeddings before being passed to the task-specific output layer. For the feasibility model, the hidden dimension, dropout rate, batch size, and learning rate are set to 256, 0.2, 128, and 0.001, respectively. Weighted binary cross-entropy is used to handle class imbalance. The screening threshold is selected on the validation set according to a target feasibility recall of 0.995.
...
\toprule
Model & ROC-AUC & PR-AUC & F1 & Reject rate & Feasibility recall & False reject count \\
\midrule
Param-MLP & 0.9117 & 0.6137 & 0.5927 & 0.4787 & 0.9946 & 90 \\
Stat-MLP & 0.9547 & 0.8163 & 0.7370 & 0.4447 & 0.9994 & 10 \\
FuseGNN & 0.9693 & 0.8593 & 0.7688 & 0.4736 & 0.9998 & 3 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\midrule
Param-MLP & 32.30 & 45.77 & 0.351 & 0.247 & -13.89 & 0.9586 & 0.9547 & 0.8974 \\
Stat-MLP & 13.83 & 18.75 & 0.891 & 0.112 & -1.66 & 0.9652 & 0.9628 & 0.9142 \\
FuseGNN & 11.48 & 15.67 & 0.924 & 0.093 & -5.02 & 0.9787 & 0.9779 & 0.9483 \\
\bottomrule
\end{tabularx}
\end{table*}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{global_surrogate_overview.pdf}
\caption{Overall performance and error patterns of the global surrogate model.}
\label{fig:global_surrogate_overview}
\end{figure*}

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{local_calibration_effect.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{calibration_sample_efficiency.pdf}
...
To examine sample efficiency, local calibration sizes of 25, 50, 100, 200, and 500 samples are considered. Each setting is repeated three times with different random samples, and the average results are reported. In the online optimization experiments, local calibration is activated only after at least 25 FEA-verified samples have been accumulated for the current layout. The feasibility screening threshold follows the same conservative principle as the global model, with a target feasibility recall of 0.995 and a threshold scale factor of 0.1. When the local samples are insufficient or contain only one class for the feasibility task, the framework falls back to the global surrogate prediction and the validation-based global threshold.

\subsection{Optimization Benchmark Settings}
\label{subsec:optimization_benchmark_settings}

The optimization experiments are conducted on 15 unseen layouts selected from the test set, comprising five layouts from each of the three design-condition groups. For each layout, five random seeds are used. For each test layout, the layout $L$ and design condition $c$ are fixed, and the optimization variables are the section and material parameters $x$. All compared methods use the same design space, true evaluator, and random seeds.

Genetic algorithm (GA) is adopted as the main optimizer\cite{Goldberg1989}. In the experiments, the population size, number of generations, mutation rate, and elite size are set to 48, 15, 0.30, and 2, respectively. To examine whether the surrogate-informed mechanism depends on a specific optimizer, particle swarm optimization (PSO) and random search are also included as supplementary optimizers\cite{Kennedy1995PSO}. The PSO uses a swarm size of 48 and 15 iterations, with a mutation rate of 0.15 to improve exploration. The random search baseline evaluates 720 candidates in batches of 48.

Three candidate evaluation strategies are compared:
\begin{enumerate}[(\arabic*)]
\item For full FEA optimization, all candidates generated by the optimizer are evaluated by $\mathcal{E}$. This strategy serves as the baseline.
\item Cost ranking: candidates are ranked using the reinforcement surrogate and the estimated concrete quantity, and only high-priority candidates are submitted to $\mathcal{E}$.
\item Screen \& cost ranking: Both feasibility screening and cost-based ranking are used to select candidates for FEA evaluation.
\end{enumerate}

For surrogate-informed strategies, each optimizer first generates a candidate batch $\mathcal{B}_t$. The surrogate module then evaluates all candidates in the batch and selects a subset $\mathcal{S}_t$ for true FEA and code checking. A minimum number of evaluated candidates is enforced to avoid overly aggressive filtering. In the experiments, the minimum number of true evaluations per batch is set to 8. The verified results are used to update the optimizer and the local calibration set.
...
To be noted, definitions of the full set of metrics used in this study are provided in \ref{app:evaluation_metrics}.

The feasibility surrogate is assessed using ROC-AUC, PR-AUC, and F1 score to characterize its overall classification performance. Since the model serves as a conservative screening tool before FEA, we further report feasibility recall, reject rate, and false reject count. These metrics quantify, respectively, the proportion of truly feasible candidates retained after screening, the proportion of candidates filtered out before FEA, and the number of feasible candidates mistakenly rejected by the surrogate.
...
Local calibration is evaluated from both probability and residual-correction perspectives. Feasibility probability calibration is measured by Brier score\cite{Brier1950} and expected calibration error (ECE)\cite{Naeini2015}, together with feasibility recall and reject rate, whereas reinforcement residual calibration is measured by MAE, RMSE, $R^2$ and MAPE after calibration.

Optimization performance is summarized in terms of computational efficiency and solution quality. Efficiency is measured by the number of true FEA calls and the FEA reduction ratio, while solution quality is characterized by feasible-solution discovery rate, the number of FEA calls required to find the first feasible solution, and final material cost. For each test layout and random seed, surrogate-informed strategies are compared with full FEA optimization to determine whether FEA calls can be reduced without substantial degradation in feasibility discovery or final cost.
