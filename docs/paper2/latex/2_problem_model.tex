...
\caption{Calibrated surrogate-informed optimization framework.}
...
\end{equation}
where $y_f \in \{0,1\}$ is the comprehensive feasibility label, $m_s$ and $m_c$ denote the reinforcement and concrete quantities, respectively, and $r$ represents structural response and checking indicators such as inter-story drift ratio, torsional response, axial compression ratio, shear-compression ratio, and member design results. A candidate is regarded as feasible only when the analysis is successfully completed and all major global and component-level checks satisfy the code requirements.
...
Given the candidate representation $z=(G,d,q)$, the proposed FuseGNN maps each candidate to pre-FEA predictions by jointly encoding its room-level graph, global layout statistics, and design parameters. Fig.~\ref{fig:layout_parameter_surrogate}(c) illustrates the model architecture. Two models are trained independently for the feasibility surrogate $\Phi_f$ and the reinforcement surrogate $\Phi_s$, respectively. The two models adopt the same input representation and network architecture but do not share parameters.

FuseGNN encodes the three components of $z$ into fixed-dimensional embeddings. The graph component $G$ is processed by a graph neural network followed by graph-level pooling, while the design vector $d$ and the global layout statistics $q$ are mapped by two separate tabular encoders. The resulting three 64-dimensional embeddings are concatenated to form the fused candidate representation $h$. This representation is used by the separately trained feasibility and reinforcement surrogate models to produce their respective predictions.
...
The reinforcement surrogate outputs the predicted reinforcement usage $\hat{m}_{s}^{G}$. This output is used as the cost-oriented ranking signal before FEA. Since concrete quantity can be rapidly approximated from geometry and section dimensions, the global surrogate cost estimate is written as
\begin{equation}
...
\begin{equation}
\mathcal{L}_f = -\omega y_f \ln \hat{p}_{f}^{G} - (1-y_f)\ln(1-\hat{p}_{f}^{G})
...
\begin{equation}
y_s=\frac{\ln(1+m_s)-\mu_s}{\sigma_s}
