...
Let $y_i \in {0,1}$ denote the true feasibility label of candidate $i$, where $y_i=1$ indicates a feasible candidate. Let $\hat{p}*{f,i}$ denote the predicted or calibrated feasibility probability, and let $\tau$ denote the screening threshold. A candidate is retained for potential FEA evaluation if
\begin{equation}
\hat{p}*{f,i} \geq \tau .
\end{equation}
...
\frac{
\sum_{i=1}^{N}
\mathbb{I}(y_i=1)\mathbb{I}(\hat{p}*{f,i}\geq \tau)
}{
\sum*{i=1}^{N}
\mathbb{I}(y_i=1)
},
...
\begin{equation}
(m_{s,i}-m_{s,j})(\hat{m}*{s,i}-\hat{m}*{s,j}) > 0 .
\end{equation}
...
Top-$k$ recall evaluates whether the surrogate can identify the truly low-reinforcement candidates. Let $\mathcal{T}_k$ denote the set of candidates with the lowest true reinforcement usage, and let $\hat{\mathcal{T}}*k$ denote the set of candidates with the lowest predicted reinforcement usage. The top-$k$ recall is defined as
\begin{equation}
R*{\mathrm{top}\text{-}k}
=
\frac{
