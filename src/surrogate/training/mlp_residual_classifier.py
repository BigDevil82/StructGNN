...
def run_mlp_residual(cfg: MLPResidualConfig) -> dict[str, object]:
    _set_seed(cfg.seed)
    ...
    test_loader = DataLoader(
        TensorDataset(torch.from_numpy(x_test_num), torch.from_numpy(x_test_cat), torch.from_numpy(y_test)),
        batch_size=cfg.batch_size,
        shuffle=False,
        num_workers=cfg.num_workers,
        drop_last=False,
    ...
    if best_state is None:
        raise RuntimeError("Residual MLP training failed to produce a valid checkpoint.")
    ...
    torch.save(
        ...
        out_dir / "clf_final_pass_mlp_residual.pt",
    )
