...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Train embedding MLP with existing GroupKFold splits and ensemble predictions."
    )
    parser.add_argument(
        "--dataset-path",
    ...
    parser.add_argument(
        "--fold-map-path",
        default=r"data\parametric\surrogate_dataset\splits\layout_group_folds.parquet",
    )
    parser.add_argument(
        "--output-dir",
...
def main() -> None:
    ...
    cfg = MLPEmbeddingKFoldConfig(
        ...
        fold_map_path=args.fold_map_path,
